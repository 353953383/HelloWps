        ...
        try {
            ...
            function tryLoadPath() {
                if (currentPathIndex >= configPaths.length) {
                    reject(new Error('无法从任何路径加载配置文件: ./server-config.txt, ../server-config.txt, /server-config.txt, server-config.txt'));
                    return;
                }
                
                const currentPath = configPaths[currentPathIndex];
                console.log(`尝试从路径加载配置: ${currentPath}`);
                
                xhr.open('GET', currentPath, true);
                xhr.timeout = 10000; // 10秒超时
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4) {
                        if (xhr.status === 200 || xhr.status === 0) { // 状态码0表示本地文件访问
                            try {
                                ...
                                if (configText) {
                                    ...
                                    // 不再验证PRODUCTION字段，直接resolve配置对象
                                    resolve(config);
                                } else {
                            ...
                            console.warn(`路径 ${currentPath} 加载失败，状态码: ${xhr.status}，尝试下一个路径...`);
                            currentPathIndex++;
                            tryLoadPath();
                        }
                    }
                ...
                xhr.onerror = function() {
                    console.warn(`路径 ${currentPath} 网络错误，尝试下一个路径...`);
                    currentPathIndex++;
                    tryLoadPath();
                };
                xhr.ontimeout = function() {
                    console.warn(`路径 ${currentPath} 加载超时，尝试下一个路径...`);
                    currentPathIndex++;
                    tryLoadPath();
                };
                xhr.send();
...
(function() {
    ...
    function applyFallbackConfig() {
        ...
        if (window.AI_CONFIG_TYPE === 'AI_CONFIG_WLAN') {
            ...
            console.log('✅ 使用应急局域网AI配置');
        } else {
            window.CURRENT_AI_CONFIG = window.AI_CONFIG;
            console.log('✅ 使用应急云端AI配置');
        }
