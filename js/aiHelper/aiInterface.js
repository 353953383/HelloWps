...
class AIInterface {
    ...
    loadAIConfig() {
        // 从全局变量读取AI配置
        const globalAIConfig = window.AI_CONFIG;
        ...
        // 验证必要配置项
        if (!globalAIConfig.apiEndpoint) {
            throw new Error('❌ AI配置错误：缺少apiEndpoint参数');
        }
        ...
        this.maxTokens = globalAIConfig.maxTokens || 2000;
        this.temperature = globalAIConfig.temperature || 0.1;
        this.timeout = globalAIConfig.timeout || 30000;
        this.requestFormat = globalAIConfig.requestFormat || 'dashscope';
     ...
     * 使用指定端点生成公式
     */
    async generateFormulaWithEndpoint(requestData, endpoint, modelName = null) {
        try {
            ...
            // 发送请求到AI模型
            const response = await this.sendAIRequest(aiRequest);
    ...
    buildAIRequest(requestData) {
        ...
        // 数据完整性检查和修复
        const safeRequestData = this.ensureDataIntegrity(requestData);
        ...
        if (this.apiEndpoint.includes('/compatible-mode/')) {
            // 兼容模式：使用OpenAI格式 + 配置的maxTokens
            requestBody = {
                model: this.modelName,
                messages: [
                    { role: "system", content: this.buildSystemPrompt(safeRequestData) },
            ...
            console.log('🎯 [调试] 使用兼容模式API，maxTokens设置为:', this.maxTokens);
            ...
            requestBody = {
                ...
                input: {
                    messages: [
                        this.buildSystemPrompt(safeRequestData),
                        userPrompt
                    ]
                },
                parameters: {
                    max_tokens: this.maxTokens || 2000,
                    temperature: this.temperature || 0.3, // 使用配置的温度值，默认0.3
                    top_p: 0.8,
                    incremental_output: false
                }
            };
            console.log('🎯 [调试] 使用标准DashScope API，maxTokens设置为:', this.maxTokens);
        }
    ...
    ensureDataIntegrity(requestData) {
        ...
        if (safeData.description === undefined || safeData.description === null) {
            console.log('⚠️ [调试] description为undefined/null，使用默认值');
            ...
            if (trimmedDesc === '') {
                console.log('⚠️ [调试] description为空字符串，使用默认智能分析描述');
...
7. 若信息不足，请直接按可能概率推荐有可能的公式`;
    ...
    async sendAIRequest(requestData) {
        ...
        try {
            ...
            throw new Error(`API端点格式无效: ${this.apiEndpoint}`);
        }
        ...
        console.log(`   模型: ${aiRequest.model}`);
        ...
        // 如果是兼容模式API，可以尝试no-cors模式
        let response;
        try {
            response = await fetch(this.apiEndpoint, fetchOptions);
        } catch (corsError) {
        ...
        try {
            ...
            if (jsonCodeBlockMatch) {
                ...
                if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
                    ...
                    jsonContent = content.substring(firstBrace, endIndex + 1);
                }
            }
            ...
            console.log('🔍 [parseAIResponse] 提取的JSON内容:', jsonContent.substring(0, 200) + '...');
            ...
            try {
                ...
                console.error('📄 [parseAIResponse] 原始内容:', content.substring(0, 500));
    ...
    validateResponse(response) {
        ...
        // 基本结构验证
        if (!response.formulas || !Array.isArray(response.formulas)) {
            throw new Error('AI响应格式错误: 缺少formulas数组');
        }
        
        // 验证每个公式的结构
        response.formulas.forEach((formula, index) => {
            if (!formula.title || !formula.formula || !formula.explanation) {
                throw new Error(`第${index + 1}个公式缺少必要字段`);
            }
            
            if (typeof formula.confidence !== 'number' || formula.confidence < 0 || formula.confidence > 100) {
                formula.confidence = 75; // 设置默认值
            }
        });
        
        // 验证其他可选字段
        if (!response.data_analysis) {
            response.data_analysis = { headers_found: [], data_types: [], recommendations: [] };
        }
